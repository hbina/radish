...
impl Client {
    ...
    async fn handle_client_connection(&mut self) {
        ...
        loop {
            // TODO(hbina) - Should we even have a timeout here? A redis-cli client that never sends anything is a perfectly reasonable client I think.
            ...
            // TODO(hbina) - How to distinguish between reading zero because we haven't received anything or zero because the client ends?
            if count == 0 {
    ...
    fn handle_cmd_config_set(&mut self, arr: RespArray) -> CommandResult {
        ...
        // TODO(hbina) - Should only allow valid keys. Also need to check the exact behavior in redis.
    ...
    fn handle_cmd_set(&mut self, arr: RespArray) -> CommandResult {
        ...
        // TODO(hbina) - Reuse this value somehow?
    ...
    fn handle_cmd_select(&mut self, arr: RespArray) -> CommandResult {
        ...
        // TODO(hbina) - Check the conversion here
    ...
    fn handle_cmd_flushall(&mut self, arr: RespArray) -> CommandResult {
        ...
        // TODO(hbina) - handle SYNC/ASYNC flushall.
        // For now we just ignore
        let _ = arr.get(1);

        let dbs_lock = self.server.dbs.lock().unwrap();

        // TODO(hbina) - this is very dangerous because we might have a deadlock!!
    ...
    fn handle_cmd_function(&mut self, _: RespArray) -> CommandResult {
        // TODO(hbina) - Just a stub implementation
        Ok(Resp::rbs_from_static_str("OK"))
    }
        ...
        if !db.contains_value(&key) {
            ...
            match db.get_expiry(&key) {
                Some(ttl) => {
                    ...
                    // TODO(hbina) - Handle conversion properly here
    ...
    fn handle_cmd_flushdb(&mut self, arr: RespArray) -> CommandResult {
        ...
        let mode = match arr.get(1) {
            Some(b) => {
                if b.as_bytes().eq_ignore_ascii_case(b"sync") {
                    true
                } else if b.as_bytes().eq_ignore_ascii_case(b"async") {
                    false
                } else {
                    return Err(CommandError::SyntaxError);
                }
