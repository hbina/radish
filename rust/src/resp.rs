...
impl Resp {
    pub fn res_from_str(input: &str) -> Self {
        return Resp::Error(Bytes::copy_from_slice(input.as_bytes()));
    }

    pub fn res_from_static_str(input: &'static str) -> Self {
        return Resp::Error(Bytes::from(input.as_bytes()));
    }

    pub fn res_from_bytes(input: &[u8]) -> Self {
        Resp::Error(Bytes::copy_from_slice(input))
    }

    pub fn rbs_from_str(input: &str) -> Self {
        return Resp::BulkString(Bytes::copy_from_slice(input.as_bytes()));
    }

    pub fn rbs_from_static_str(input: &'static str) -> Self {
        return Resp::BulkString(Bytes::from(input.as_bytes()));
    }

    pub fn rbs_from_bytes(input: &[u8]) -> Self {
        Resp::BulkString(Bytes::copy_from_slice(input))
    }

    pub fn rbs_from_static_bytes(input: &'static [u8]) -> Self {
        Resp::BulkString(Bytes::from(input))
    }

    pub fn rss_from_static_bytes(input: &'static [u8]) -> Self {
        Resp::String(Bytes::from(input))
    }
    ...
    pub fn as_bytes(&self) -> &[u8] {
        match self {
            ...
            Resp::Integer(b) => todo!("Cannot return a temporary string here which is unfortunate"),
            Resp::BulkString(b) => b.as_ref(),
            Resp::NilBulk => b"-1", // TODO(hbina) - What should be returned here?
            Resp::Array(a) => a.1.as_ref(),
            Resp::NilArray => b"-1", // TODO(hbina) - What should be returned here?
        }
    }
...
pub struct RespArray(pub usize, pub Bytes);
...
impl RespArrayBuilder {
    ...
    pub fn build(self) -> RespArray {
        RespArray(self.0, self.1.freeze())
    }
}
...
pub struct RespArrayIterator(usize, Bytes);
...
impl Iterator for RespArrayIterator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        self.0 += resp_length;
        Some(
            cast_bytes_to_resp(&Bytes::from(resp_bytes))
                .expect("we should have checked that every construction of RESP is valid"),
        )
    }
}
...
pub fn cast_bytes_to_resp(input: &Bytes) -> RespResult {
    let marker_byte = input[0];
